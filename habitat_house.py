...
scene = "Habitat_House_01"
controller = Controller(agentMode=AGENT, branch="nanna_habitat_house", scene=scene)

MAX_REACHABLE_POSITIONS: int = 250
TASKS_PER_OBJECT_TYPE_PER_SCENE: int = 100
...
for obj_type in TARGET_OBJECT_TYPES:
    ...
    for i, position in enumerate(reachable_positions):
        ...
        for object_id in object_ids:
            ...
            path_length = l2_distance(obj["axisAlignedBoundingBox"]["center"], position)
        ...
        for min_length, starting_position in agents_starting_data:
            ...
            task = {
                ...
                "agentPose": AgentPose(
                    ...
                    rotation=int(random.choice(np.arange(0, 360, ROTATE_STEP_DEGREES))),
                    horizon=30,
                    **standing,
                ),
...
# save tasks as a json lines file
with open(f"datasets/habitat-{AGENT}.jsonl", "w") as f:
    for task in tasks:
